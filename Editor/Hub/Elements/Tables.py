...
class ProjectTable():
    ...
    def Local():
        ...
        def GetProjectTableHeaders():
            print(Fore.BLUE +"Instantiating " + "Editor.Hub.Configs" + " to get ProjectTableHeaders...")

            result = subprocess.run(['python', '-m', 'Editor.Hub.Configs', '--GetProjectsTableHeaders'], capture_output=True, text=True)
        ...
        def GetProjectListPath():
            print(Fore.BLUE +"Instantiating " + "Editor.Hub.Configs" + " to get ProjectListPath...")
            result = subprocess.run(['python', '-m', 'Editor.Hub.Configs', '--GetProjectListPath'], capture_output=True, text=True)
...
class Buttons(QPushButton):
    class Star(QPushButton):
        ...
        def buttonClicked(self, project):
            def SetStar(projectID, starred):
                ...
                result = subprocess.run(["python", "-m", "Editor.Hub.Configs"] +
    ...
    class Play(QPushButton):
        ...
        def buttonClicked(self, project):
            # TODO THIS MAKES IT OPEN ONLY THE PONG PROJECT
            # subprocess.run(["python", "-m", "Editor.Editor"] + ["OpenProject","--path", self.activeProjectPath])
            subprocess.run(["python", "-m", "Editor.Editor"] + ["OpenProject","--path", "/home/admin/github/bleue-mer/Projects/dvd_bouncer"])
